...
const Technicians = () => {
  const [technicians, setTechnicians] = useState([]); // Initially empty, fetched from backend
    ...
    const [formData, setFormData] = useState({
          ...
          address: "",
          DateOfBirth: "",
          role: "LabTech", // Default role for technicians
          age: "", // Add age field
    ...
    const fetchTechnicians = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log("Fetched technicians:", data); // Log the fetched data
          setTechnicians(data); // Update the Technicians state with the fetched data
        } else {
          const errorData = await response.json();
          console.error("Error fetching technicians:", errorData); // Log the error details
          toast.error(errorData.message || "Failed to fetch technicians.");
        }
      } catch (error) {
    ...
    const currentUsers = sortedTechnicians.slice(indexOfFirstUser, indexOfLastUser);
  
    const totalPages = Math.ceil(sortedTechnicians.length / usersPerPage);
         ...
         const handleSubmit = async (e) => {
            ...
            try {
              ...
              if (response.ok) {
                ...
                toggleModal();
                fetchTechnicians(); // Refresh the list of technician
              } else {
                const errorData = await response.json();
                toast.error(errorData.message || "Failed to update/add technician.");
              }
            } catch (error) {
              toast.error("An error occurred while updating/adding the technician.");
            }
          };
  
    const handleView = (id) => {
      const technician = technicians.find((technician) => technicians.id === id);
      toast.info(`Viewing details for ${technicians.fullname}`);
    };
    ...
    return (
      ...
      <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
        <h1 className="text-xl font-bold whitespace-nowrap">Technician List</h1>
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full">
        ...
        <tbody>
          {currentUsers.map((technician, index) => (
            <tr key={technician.id} className="hover:bg-gray-50 transition-colors duration-200">
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{indexOfFirstUser + index + 1}</td>
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{technician.fullname}</td>
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{technician.username}</td>
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{technician.email}</td>
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{technician.address}</td>
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{technician.Salary}</td>
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {technician.Contract && (
                  <a
                    href={`http://localhost:5000/uploads/${technician.Contract}`}
                    target="_blank"
              ...
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{technician.age}</td>
              <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{technician.role}</td>
                          ...
                          {showModal && (
                            ...
                            <div className="bg-white rounded-xl w-11/12 lg:w-8/12 h-[80vh] overflow-hidden shadow-lg transform transition-all duration-300 flex flex-col">
                              <div className="border-b border-stone-200 p-4 sm:p-5 flex justify-between items-center bg-gray-100">
                                <h1 className="text-lg font-semibold text-stone-800">+Add Technician</h1>
