...
const Cashiers = () => {
  const [cashiers, setCashiers] = useState([]); // Initially empty, fetched from backend
    ...
    const [formData, setFormData] = useState({
          ...
          address: "",
          DateOfBirth: "",
          role: "Cashier", // Default role for cashier
          age: "", // Add age field
    ...
    const handleSubmit = async (e) => {
            ...
            try {
              ...
              if (response.ok) {
                ...
                toggleModal();
                fetchCashiers(); // Refresh the list of cashier
              } else {
                const errorData = await response.json();
                toast.error(errorData.message || "Failed to update/add cashier.");
              }
            } catch (error) {
              toast.error("An error occurred while updating/adding the cashier.");
            }
          };
