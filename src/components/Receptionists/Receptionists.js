...
const Receptionists = () => {
  const [receptionists, setReceptionists] = useState([]); // Initially empty, fetched from backend
    ...
    const [formData, setFormData] = useState({
      ...
      address: "",
      DateOfBirth: "",
      role: "Reception", // Default role for receptionists
    });
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          toggleModal();
          fetchReceptionists(); // Refresh the receptionist list
        } else {
          const errorData = await response.json();
          toast.error(errorData.message || "Failed to add receptionist.");
        }
      } catch (error) {
    ...
    return (
      ...
      >
        ...
        Delete
          ...
          {/* Modal for Adding/Editing Receptionists */}
          {showModal && (
