...
const RchClinics = () => {
  const [rchclinics, setRchclinics] = useState([]);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (response.ok) {
        toast.success("RCH Clinic deleted successfully!");
        fetchRchclinics();
      } else {
        const errorData = await response.json();
      ...
      toast.error("An error occurred while deleting the RCH Clinic.");
    }
  };
  ...
  return (
    <div className="p-4 sm:p-6 bg-gray-100 rounded-lg shadow-md mt-5">
      ...
      <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
        <h1 className="text-xl font-bold whitespace-nowrap">RSHClinic List</h1>
            ...
            <tbody>
              {currentUsers.map((rchclinic, index) => (
                <tr key={rchclinic.id} className="hover:bg-gray-50 transition-colors duration-200">
                  ...
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{rchclinic.fullname}</td>
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{rchclinic.username}</td>
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{rchclinic.email}</td>
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{rchclinic.address}</td>
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{rchclinic.Salary}</td>
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {rchclinic.Contract && (
                      <a
                        href={`http://localhost:5000/uploads/${rchclinic.Contract}`}
                        target="_blank"
                  ...
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{rchclinic.age}</td>
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{rchclinic.role}</td>
      ...
      {/* Modal for Adding/Editing RCH Clinic */}
      {showModal && (
          ...
          <div className="bg-white rounded-xl w-11/12 lg:w-8/12 h-[80vh] overflow-hidden shadow-lg transform transition-all duration-300 flex flex-col">
            <div className="border-b border-stone-200 p-4 sm:p-5 flex justify-between items-center bg-gray-100">
              <h1 className="text-lg font-semibold text-stone-800">
                {formData.id ? "Edit RCH Clinic" : "+Add RCH Clinic"}
              </h1>
